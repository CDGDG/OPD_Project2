$(document).ready(function(){
    ...
    $('#form_id input').css({'box-shadow': 'none'});
    ...
    $('#form_id textarea').siblings('label').css({'height':'100%', 'padding': '8% 0 !important'})
    ...
    $('#form_id input[type="date"]').css({'padding': '2vh', 'display': 'none', 'border': '1px solid lightcoral'}).siblings('label').removeClass('py-4')
        .css({'position': 'static','padding':'3vh'}).parent().addClass('col-6 pe-2').css({'display': 'inline-block'})

    // 비공개
    $('#form_id .form-check').css({'height': '100%'}).addClass('px-0').parent().addClass('col-6').css({'display': 'inline-block'})
    $('#form_id input[type="checkbox"]').hide().siblings('label').removeClass('py-4').addClass('labelP5').css({'height': '100%'})

    // 언어
    $("#form_id #id_language").removeClass('col-6').addClass('col-12').siblings('label').addClass('label-color py-2 mb-1').removeClass('py-4').parent().find('label').css({'position':'static'})
    ...
    $('#form_id input[type="date"]').siblings("label").click(function(){
        ...
        $('#form_id input[type="checkbox"]').siblings('label').toggleClass('labelP10')
    })
